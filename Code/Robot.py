...
"""
le robot du jeu, contiendrat toutes les informations, et traduira les chaines que nous renvera le serveur
"""
...
class Robot:
    ...
    def analyseMessage( self, state ):
        ...
        :param str state: la chaine reçu, envoyé par le serveur, peut contenir un message STATE, GAMEOVER, ou ENDOFGAME 
        """
        ...
        if( state_of_game.match( state ) ):
            ...
            raise RobotException("je ne comprend pas ce que c'est, ne correspond pas au protocole : {quezako} ".format( quezako=state ) )
    ...
    def game_over( self, state_game_over ):
        """
        L'un des participants au match a perdu, on analyse plus en détail la chaine reçu pour savoir si c'est ce robot qui ne peut plus jouer.
